function startDraw(canvasId, rangeId) {
	...
	$("#clearBtn").click(function(){
		drawing.clearCanvas();
		drawing.recordings[0].pauseInfo = null;			
	});
...
RecordableDrawing = function (canvasId, rangeId)
{
	var self = this;
	this.canvas = null;
	this.range = null;
...
Recording = function (drawingArg)
{
	...
	this.playRecording = function(callbackFunctionArg, onPlayEnd, onPause, interruptActionStatus)
	{
		...
		var actionSet = self.actionsSet;

		if(self.pauseInfo){
			actionSet = self.pauseInfo.actionset;
		}

		self.scheduleDraw(actionSet,self.actionsSet.interval,callbackFunctionArg, onPlayEnd, onPause, true, interruptActionStatus);
	}
