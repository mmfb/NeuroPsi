...
create table Test (testId int auto_increment primary key, testState enum("Pending", "Completed", "Filed", "Reschedule", "Canceled") default "Pending", assignedDate date, test_attribId int, test_routeId int, foreign key (test_attribId) references Attribution (attribId), foreign key (test_routeId) references Route (routeId));
create table Result (resultId int auto_increment primary key, rec longtext, completedDate date, comment varchar(300), result_testId int, foreign key (result_testId) references Test (testId));
